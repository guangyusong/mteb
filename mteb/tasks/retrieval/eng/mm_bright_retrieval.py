...
def _load_multimodal_queries(domain: str, examples: Dataset) -> Dataset:
    ...
    for example in examples:
        ...
        rows.append({"id": example["id"], "text": example["query"], "image": images})
    ...
    no_image_table = pa.table(
        {
            "id": no_image_ids,
            "text": document_texts,
            "image": pa.nulls(len(documents), type=_IMAGE_FEATURE.pa_type),
        }
    )
...
class _MMBrightRetrieval(AbsTaskRetrieval):
    ...
    def load_data(
        ...
        **kwargs,
        ...
        if self.data_loaded:
            return
        self.dataset = {
            domain: {"test": _load_domain(domain, self.variant)}
            for domain in self.hf_subsets
        }
        self.data_loaded = True


class MMBrightT2TRetrieval(_MMBrightRetrieval):
    variant = "t2t"
...
class MMBrightIT2TRetrieval(_MMBrightRetrieval):
    variant = "it2t"
    metadata = TaskMetadata(
...
class MMBrightIT2IRetrieval(_MMBrightRetrieval):
    variant = "it2i"
    metadata = TaskMetadata(
...
class MMBrightIT2ITRetrieval(_MMBrightRetrieval):
    variant = "it2it"
    metadata = TaskMetadata(
